...
| Moeda | Não verificável |
| Método de importação | **Indefinido — spike técnico necessário** |
| Sitemap | Não declarado; `/sitemap.xml` retorna 404 |
| Robots | Permissivo (`Allow: /`), só bloqueia `/checkout/`, `/cambiodeldia/`, uma referência externa |
| Canonical Coverage | N/A |
| Estimated Products | Não verificável |
| Estimated Offers | Não verificável |
| Estimated Sync Time | N/A até spike |
| Sync Frequency | Provável diária (alto giro de SKU típico de importador), a confirmar |
| Connector Complexity | **Média-Alta** |
| Known Risks | URLs de produto indexadas pelo Google retornam 404 em fetch direto; homepage retorna apenas um shell quase vazio sem nav/scripts — forte indício de SPA renderizada via JS (blog no mesmo domínio, por contraste, é 100% server-rendered, confirmando que a ferramenta de fetch funciona). Sem sitemap, descoberta de catálogo dependeria de crawling de categoria ou engenharia reversa de uma API JSON interna (comum em lojas OpenCart, stack sugerida pelas URLs legadas `?product_id=X&route=product/product`) |
| Certification Status | **Needs Technical Spike** (não bloqueado por política — apenas tecnicamente não confirmado) |
| Integration Strategy | Pending Decision |
| Observações | Recomenda-se um spike curto com Playwright/DevTools antes de estimar esforço — a diferença entre "precisa de headless browser" e "tem API JSON explorável" muda a complexidade de Média-Alta para Baixa. |
...
| Método de importação | **Indefinido — spike técnico necessário** |
| Sitemap | Sim, mas contém apenas 24 URLs utilitárias/estáticas (contato, frete, wishlist) — **nenhuma URL de produto/categoria** |
| Robots | Permissivo (`Allow: /`), com `Host:` e `Sitemap:` declarados |
| Canonical Coverage | N/A |
| Estimated Products | Não verificável via sitemap; catálogo real precisa ser descoberto via `/busca/categoria/{nome}/{id}/` |
| Estimated Offers | Não verificável |
| Estimated Sync Time | N/A até spike |
| Sync Frequency | Diária, a confirmar |
| Connector Complexity | **Média-Alta** |
| Known Risks | Página de produto retornou apenas status de estoque ("Esgotado") em HTML estático — preço não apareceu no fetch estático, sinal de que o bloco de preço/grid de produto pode depender de hidratação client-side (JS). Recomenda-se confirmar com um spike de navegador real (Playwright) antes de comprometer arquitetura — o fetch estático pode estar truncando conteúdo em vez de provar renderização client-side |
| Certification Status | **Needs Technical Spike** |
| Integration Strategy | Pending Decision |
| Observações | App Next.js — mesma stack de Atacado Connect, mas com catálogo não descobrível via sitemap, ao contrário daquele. |
