...
6. **Mesmo "aprovar" nunca é automático de verdade sem verificação prévia.** Mesmo no caminho de auto-aprovação (`ClaimService.create`, quando `autoApprovable=true`), a aprovação passa pelo mesmo código (`storeLinkRepo.link` + `verificationService.approveVerification`) que o caminho manual do admin usa — não existe um atalho que pule a verificação, só um atalho que dispensa a espera por um humano quando a confiança já é suficientemente alta.
   ...
   **Correção (Wave 6, 2026-07-02, decisão do CTO):** o atalho de auto-aprovação descrito acima foi **desativado**. Auditoria da Wave 6 encontrou que todos os sinais que `ProgressiveVerificationEngine` compara (telefone, WhatsApp, website, Instagram, domínio de e-mail) também são exibidos publicamente em `/lojas/[slug]` — um requerente sem nenhuma relação real com a loja podia copiar esses valores da própria página pública e atingir o limiar de auto-aprovação (80%) com telefone+WhatsApp sozinhos (45 de peso aplicável, o piso mínimo), derrubando exatamente a garantia anti-fraude que este item descreve. `ClaimService.create` agora sempre cria a claim como `AwaitingReview`; `result.autoApprovable`/`result.confidence` continuam computados e persistidos como evidência para o revisor humano, mas nunca mais pulam a revisão. Restaurar um caminho rápido de verdade exige um sinal privado real (OTP para telefone/e-mail da loja) — fora do escopo de uma wave de hardening, nomeado como prioridade para o Release 1.8 em `docs/operations/RELEASE_CERTIFICATION_1.7.md`.
