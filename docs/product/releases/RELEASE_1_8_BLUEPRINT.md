...
**Wave 6 — Buyer Account System & Experience** (Capítulo 6). A Wave mais sensível deste Release (dados pessoais, autenticação nova, Tipo 1) — recomenda-se dividir internamente em 6a (contas + favoritos + comparações + histórico) e 6b (alertas + notificações + Compra Inteligente, que dependem de Waves 1–2 já estarem maduras). **Modelo de dados pessoais já decidido — ADR-045**: LGPD como referência (alcance extraterritorial confirmado, aplicável desde o primeiro comprador brasileiro), tabela `buyers` própria (não reaproveita `profiles`, que é staff interno), deleção por anonimização preservando sinal agregado. Dois fixes companheiros obrigatórios antes do cadastro público: corrigir `handle_new_user()` (hoje rotula todo signup como `'operator'`) e migrar `merchant_reviews.reviewer_id` de `profiles` para `buyers`. Designação de Encarregado (LGPD Art. 41) e texto de política de privacidade continuam ação do CTO/jurídico, fora do escopo técnico. **ADR ainda necessário antes de começar**: fornecedor de notificação (push/e-mail).
...
**Risco estratégico a documentar antes do encerramento, conforme exigido**: este Blueprint originalmente dependia de três decisões Tipo 1 que não haviam sido tomadas e bloqueavam Waves específicas. **Duas já foram resolvidas**: fornecedor de câmbio (Wave 1) — ver ADR-043, ExchangeRate-API.com plano Business — e modelo de dados pessoais de comprador (Wave 6) — ver ADR-045, LGPD como referência vinculante real (alcance extraterritorial, sem limiar mínimo — um único comprador brasileiro já basta), tabela `buyers` própria, deleção por anonimização sem destruir sinal agregado (`C-6`). ADR-045 também expôs dois gaps de schema pré-existentes que precisam ser corrigidos junto: `handle_new_user()` rotula hoje todo novo cadastro como `'operator'` (staff interno), e `merchant_reviews.reviewer_id` referencia `profiles` sem nunca ter sido de fato alimentado por um comprador real. **Uma permanece em aberto**: fornecedor de billing (Wave 8) — mais o fornecedor de notificação push/e-mail para a Wave 6, um sub-item menor não coberto pelas ADRs 043/045. Nenhuma é hipotética ou adiável indefinidamente — são decisões que o CTO precisa tomar, com ADR formal, antes que as Waves correspondentes possam começar. **Adicionalmente, ADR-045 nomeia uma ação fora do escopo de engenharia**: designação de Encarregado (LGPD Art. 41) e revisão jurídica real da política de privacidade — nenhuma ADR de engenharia substitui aconselhamento jurídico formal.
