...
> **✅ Atualização 2026-07-03 — Program A Wave 3 (auditoria) + Program C Wave 0 (parcerias)**: a auditoria técnica das 10 lojas Tier 1 (`docs/marketplace/Tier1_Merchants.md`) encontrou que 4 delas — Cellshop, Nissei, Casa Americana, New Zone — bloqueiam explicitamente crawlers de IA no `robots.txt`, tornando "Viabilidade técnica" (25pts acima) estruturalmente zero para scraping, mas não para o negócio: o Merchant Partnership Program (`docs/business/MERCHANT_PARTNERSHIP_PROGRAM.md`) abre um segundo caminho de entrada de dado (feed/API oficial) que não depende deste score de viabilidade técnica de scraping. Nenhuma dessas 4 lojas foi despriorizada — seguem Tier 1, apenas com `Integration Strategy: Data Partnership` em vez de `Public Connector`.
...
**Regra permanente, decisão já aprovada pelo CTO**: "nunca substituir o preço original." Isso significa: o preço original nunca é sobrescrito por uma conversão — a conversão é sempre um valor **derivado, calculado no momento da leitura**, nunca substitui `price_usd`/`price_brl` originais na oferta.

> **Correção pós-implementação (Program A — Wave 1, 2026-07-03)**: este capítulo propunha uma coluna nova `original_currency` em `offers`/`price_history`. Pesquisa de schema feita durante a implementação encontrou que `offers.currency` **já existe** e já significa exatamente isso — todo conector já preenche esse campo (`"USD"` por padrão hoje). Criar uma segunda coluna para o mesmo conceito seria duplicação. Decisão tomada e documentada em `docs/operations/DECISIONS.md` (ADR — Exchange Intelligence Platform): `offers.currency` é reaproveitada como "moeda original", nenhuma coluna nova foi criada. `rateUsed`/`convertedPrice`/`conversionDate`/`rateVersion` continuam exatamente como proposto abaixo — computados sob demanda, nunca persistidos.
...
> **✅ Entregue 2026-07-03 como "Real-Time Commerce Engine"** — o CTO re-escopou esta Wave (mesma classe de decisão que re-escopou as Waves 4/5 do Release 1.7): em vez do refresh tiered por oferta descrito acima (que continua não construído), o mandato real desta Wave foi construir a **camada de detecção e observabilidade** que antecede o refresh acelerado — Change Detection Engine (append-only, integrado de fato ao `SyncOrchestrator`), Volatility Engine, Freshness Engine (Score/classificação já implementados — apenas o tiering hot/warm/cold do Capítulo 2 e a aceleração de cadência de sync em si não foram construídos), Store Update Intelligence, Market Pulse e a fundação do Buyer Alert Engine. C-7 (Live Commerce Velocity) e Moat 9 avançam de "não iniciado" para "instrumentado", exatamente como este capítulo previa que aconteceria — só que pela via de detecção de mudança em vez de refresh tiered. Ver `docs/operations/CHANGELOG.md` (2026-07-03, Program A Wave 2) para o relatório completo, e `docs/engineering/TECH_DEBT.md` para os gaps nomeados (`ProductRemoved`/`OfferRemoved` não detectados, emissão real dos 10 novos `TrustEventType` deferida). O refresh tiered por oferta em si (a parte "atualizar preços" deste capítulo) permanece arquitetura definida, não construída — próxima candidata a Wave do Programa A se o CTO optar por reabri-lo.
...
**Wave 6 — Buyer Account System & Experience** (Capítulo 6). A Wave mais sensível deste Release (dados pessoais, autenticação nova, Tipo 1) — recomenda-se dividir internamente em 6a (contas + favoritos + comparações + histórico) e 6b (alertas + notificações + Compra Inteligente, que dependem de Waves 1–2 já estarem maduras). **Buyer Identity Model completo e definitivo — ADR-045 + ADR-046** (`docs/product/releases/RELEASE_1_8_BUYER_IDENTITY_MODEL.md`, 15 seções): domínio próprio (`buyers` como aggregate root, nunca `profiles` — que é staff/operator/merchant por decisão deliberada da ADR-031 — nem `auth.users` diretamente), ciclo de vida de 6 estados (Anônimo → Sessão Anônima → Conhecido [verificado/não] → Autenticado → Recorrente [rótulo derivado] → Premium [futuro]) + Anonimização terminal (LGPD), pseudonimização controlada por RLS separando o Brain de qualquer PII. Auditoria expôs 3 dívidas pré-existentes (não corrigidas, nomeadas para a Wave 6): `handle_new_user()` rotula todo signup como `'operator'`; `buyer_events`/`buyer_sessions.buyer_id` acopla-se a `auth.users` em vez de a um domínio de comprador; `merchant_reviews.reviewer_id → profiles` nunca foi alimentado por um comprador real. Também expôs **2 achados de segurança de severidade alta já em produção**: `buyer_events`/`buyer_sessions` aceitam INSERT anônimo irrestrito (comentário do código alega rate limiting que a auditoria da própria Wave 6 do Release 1.7 já provou não existir), e a coleta comportamental já roda desde o Release 1.6 sem nenhum aviso de privacidade/consentimento — ambos recomendados para correção prioritária, não adiamento. Designação de Encarregado (LGPD Art. 41) e texto de política de privacidade continuam ação do CTO/jurídico. **Achado do Sprint 0.1, resolvido no Program 0 Wave 0** (`docs/product/releases/RELEASE_1_8_PROGRAM_0_WAVE_0_REPORT.md`): as migrations `0018`-`0021` (base de dados de `buyer_events`) nunca tinham sido aplicadas em produção — corrigido no Sprint 0.1. O Brain não recebia nenhum desses eventos — corrigido no Wave 0: `BuyerEventBrainBridgeService` bridgeia eventos com `merchant_id` resolvido para `merchant_trust_events`, verificado end-to-end contra produção. A maturação de `C-6` continua limitada não pelo mecanismo (agora real) mas pelo volume — zero lojas reivindicadas em produção hoje, achado do próprio Wave 0. **ADR ainda necessário antes de começar**: fornecedor de notificação (push/e-mail) — único item de Wave 6 sem decisão própria ainda.
...
**Nota — Program 0 (fora deste Blueprint)**: assim como Program 0 Wave 0 (Brain Analytics Integration, referenciado no Capítulo 12 acima), **Program 0 Wave 1 — Marketplace Operations Platform** (2026-07-03) roda em paralelo à sequência A-F deste Blueprint, sem ocupar nenhuma das 8 Waves numeradas acima — sua própria numeração de Wave (0, 1, ...) é intencionalmente separada. Entrega: domínio `src/domains/marketplace-operations/` (Health Score, Merchant Priority Engine, Coverage Engine, Connector Health Engine estendendo o Ecosystem Monitor da Wave 2/Release 1.7, Marketplace Metrics, Alert Engine, dashboard `/admin/marketplace-operations`). Ver `docs/product/ROADMAP_1_8.md` (seção PROGRAM 0) para o resumo executivo e `docs/operations/PROJECT_STATUS.md`/`docs/operations/CHANGELOG.md` para o detalhamento completo.
...
**Risco estratégico a documentar antes do encerramento, conforme exigido**: este Blueprint originalmente dependia de três decisões Tipo 1 que não haviam sido tomadas e bloqueavam Waves específicas. **Duas já foram resolvidas**: fornecedor de câmbio (Wave 1) — ver ADR-043, ExchangeRate-API.com plano Business — e o Buyer Identity Model completo (Wave 6) — ver ADR-045 + ADR-046 (`RELEASE_1_8_BUYER_IDENTITY_MODEL.md`), que não só define o modelo de dados pessoais mas o domínio inteiro (aggregate root, ciclo de vida, identificadores, separação Brain/PII). Essa auditoria mais profunda (ADR-046) expôs, além dos dois gaps de ADR-045, dois achados de segurança de severidade alta **já em produção hoje**: ausência de rate limiting real em `buyer_events`/`buyer_sessions` e ausência de aviso de privacidade para coleta comportamental que já roda desde o Release 1.6 — nenhum dos dois foi causado pelo Buyer Identity Model, mas ambos foram revelados por auditá-lo, e nenhum deveria esperar o fim da Wave 6 para ser corrigido. **Uma permanece em aberto**: fornecedor de billing (Wave 8) — mais o fornecedor de notificação push/e-mail para a Wave 6, um sub-item menor não coberto pelas ADRs 043/045/046. Nenhuma é hipotética ou adiável indefinidamente — são decisões que o CTO precisa tomar, com ADR formal, antes que as Waves correspondentes possam começar. **Adicionalmente, ADR-045/046 nomeiam uma ação fora do escopo de engenharia**: designação de Encarregado (LGPD Art. 41) e revisão jurídica real da política de privacidade — nenhuma ADR de engenharia substitui aconselhamento jurídico formal.
