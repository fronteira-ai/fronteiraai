...
7. **Offer Ranking nunca usa Reputation Score.** Restrição permanente desde o Release 1.5 (`docs/operations/RELEASE_CERTIFICATION_1.5.md`: "Zero Reputation Score... nenhum componente calcula ou exibe score de reputação"). O fator "trust" do `OfferRankingService` é um booleano explícito (`stores.is_verified`, coluna já existente), resolvido pelo chamador — o serviço nunca computa nem consulta um score de reputação.
