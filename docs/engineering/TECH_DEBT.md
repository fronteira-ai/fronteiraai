...
- ~~`CRON_SECRET`/`CRON_APP_URL` ainda não configurados no GitHub Actions~~ — **resolvido**: ambos configurados (confirmado por execução real, não apenas leitura de Settings — 447+ runs desde 2026-07-06). Achado real e distinto encontrado depois (2026-07-27 a 2026-07-30, Mission Ω-Merge-Suggestions Hotfix + Ω-GitHub Final Hotfix): `/api/cron/canonical-catalog/merge-suggestions` estourava o próprio `maxDuration=60` da rota (backlog real de ~7.700 itens nunca antes exercitado), e o timeout de 20s do cliente (`call-cron-endpoint.sh`) era curto demais para a duração legítima de ~46-52s do endpoint já corrigido — os dois corrigidos e validados com execução real em produção (run GitHub Actions #475, `SUCCESS`, commit `80abeeee`). Ver `docs/operations/POST_MISSION_REVIEW_OMEGA_HARDENING.md` e `docs/operations/PROJECT_STATUS.md` (entrada de fechamento de infraestrutura, 2026-07-30).
