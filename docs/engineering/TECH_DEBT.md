...
- **Novo achado, não corrigido**: o Brain **não recebe** nenhum `buyer_events` — confirmado por grep cruzado entre `src/domains/merchant-analytics/` e `src/domains/trust/`, zero referência em qualquer direção. Não é apenas o gap de mapeamento já documentado (21 `TrustEventType` sem `TRUST_EVENT_BRAIN_IMPACT`, achado da Wave 4) — é mais fundamental: nenhum código chama `EventService.recordEvent`/`CognitiveBrainService` a partir de um `buyer_events` real. Construir essa ponte é arquitetura nova de verdade (decidir quais eventos viram eventos do Brain, em que limiar, mapeados para qual `TrustEventType`) — fora do mandato "sem arquitetura nova" deste Sprint. Candidato a uma Wave própria.
