...
## 2026-09-21 — P2 PUBLIC CATALOG VISIBILITY — INACTIVE STORE LEAKAGE (PROD)

**Objetivo**: fechar o vazamento de loja/oferta inativa em superfícies públicas de consumidor (`/search`, `/lojas/[slug]`, `/product/[slug]`), reproduzido em produção pelos tenants QA, e registrar o ciclo completo: correção → migration → regressão pós-deploy → hotfix → validação pública final → E2E.

- **Contrato P2**: PUBLIC STORE = `stores.active = true`; PUBLIC OFFER = `offers.available = true` **AND** `stores.active = true`; PUBLIC PRODUCT = possui ≥1 PUBLIC OFFER. `stores.is_verified` **não** participa da regra. A elegibilidade é aplicada **antes** de preço, estoque, ranking, contagem e paginação. Visibilidade merchant/admin/internal de loja/oferta inativa segue independente (é visibilidade de consumidor, **não** autorização).
- **`dd5b6d2` (2026-09-19) — correção principal**: propagou o contrato por `stores-public`, `store`, `offer`, `product`, `search`, `home premium`, `compare`, `canonical catalog`, `buyer intelligence`, `market intelligence` e `price intelligence` (20 arquivos; inclui `services/__tests__/public-catalog-visibility.test.ts`). No catálogo público, elegibilidade/contagem/paginação passaram a ser resolvidas **no SQL** (RPC), não corrigidas em JavaScript depois da página.
- **Migration** `supabase/migrations/20260916120000_public_catalog_visibility.sql` (forward-only, novo arquivo): recria `public.search_products_catalog` e `public.search_products_global` com `JOIN stores s ON s.id = o.store_id AND s.active = true` no conjunto elegível — antes da agregação e do `count(*) OVER ()` — preservando assinatura/argumentos, `RETURNS TABLE`, `LANGUAGE sql`, `STABLE`, `SECURITY INVOKER`, ausência de `SET search_path`, `COMMENT ON FUNCTION` e `GRANT EXECUTE`. **Aplicada manualmente no PostgreSQL self-hosted de produção** (aplicação única — **não reaplicar**). Migrations históricas intactas.
- **Validação da migration em produção (read-only, executada pelo owner)**: `qa-e2e-store-a` (`active=false`, store id `c86290d9-625e-4ed0-bf7e-db893652c98f`) tem **5 ofertas `available=true`** e, ainda assim, `search_products_global('QA E2E', 20, 0)` = **0 rows** e `search_products_catalog(NULL, NULL, 'c86290d9-…'::uuid, NULL, false, NULL, NULL, 'price_asc', 20, 0)` = **0 rows**. Cenário positivo (lojas ativas): `newest`/`price_asc`/`price_desc` = 12 rows com `total_count = 43407`, confirmando eligibility/count/pagination consistentes para o conjunto público.
- **Regressão pós-deploy**: `/lojas/qa-e2e-store-a` e `/product/qa-e2e-product-005` já estavam protegidos, mas `/search?q=QA E2E` **ainda exibia os 5 produtos QA**. Diagnóstico: a origem **não** era o banco (a RPC já devolvia `[]`); em `services/search.service.ts`, a RPC bem-sucedida com `[]` só era aceita como definitiva quando `ranked.length > 0` ⇒ o código caía no caminho legado, cuja elegibilidade de loja no relacionamento não era estruturalmente inner.
- **`500f1f0` (2026-09-21) — hotfix**: (1) RPC bem-sucedida retornando `[]` ⇒ `return []` imediato, sem fallback (fallback passa a existir somente para indisponibilidade/erro da RPC); (2) enrichment `stores(active)` → `stores!inner(active)`; (3) fallback `stores(active)` → `stores!inner(active)`. O teste que codificava o comportamento antigo foi substituído e um teste de contrato do embed foi adicionado.
- **Gates (re-executados no registro desta entrada, em `500f1f0`)**: **172/172 suítes, 1264/1264 testes** PASS · `npx tsc --noEmit` PASS · `npm run lint` **0 erros** (1 warning pré-existente em `scripts/comparison-forensics-audit.ts:19`) · `git diff --check` limpo · `npm run build` PASS (Next.js 16.2.9; 150/150 páginas estáticas).
- **Deploy**: Vercel deployment `dpl_9Uarx7NpepsjsSzrozMqBgcsNbnq`, alias `https://www.fronteiraai.com`, commit em produção `500f1f0`.
- **Regressão pública final**: `GET /search?q=QA%20E2E` → "Nenhum resultado para \"QA E2E\"" (os 5 produtos QA não aparecem); `/lojas/qa-e2e-store-a` e `/product/qa-e2e-product-005` → corpo renderiza "não encontrada" e o payload contém `NEXT_HTTP_ERROR_FALLBACK;404`. O HTTP 200 externo é transporte/streaming do Next; **não** deve ser lido isoladamente como exposição.
- **E2E final (harness de 25 checks)**: execução automatizada **24/25 PASS** (checks 01–14 e 16–25). O check `15_Stock_semantics` foi diagnosticado como **falso negativo do harness**: comparava `String(r.price_usd) === '120.00'` enquanto o PostgREST serializa `numeric` sem zeros à direita (`"120"`, `"50"`); com `Number(r.price_usd) === 120` / `=== 50` a asserção é satisfeita pelos dados já capturados (`120 → in_stock=true, quantity=10` · `50 → false, 0` · `75 → false, null` · `200 → true, 5` · `30 → true, 2`). **Não houve segunda execução completa do E2E** — o check 14 grava em `price_history` (`status=200, writes=1, delta=1`), e repetir seria mutação desnecessária.
- **Follow-ups não bloqueantes** (nenhum alterado nesta janela): default `fail-open` residual em helpers equivalentes (`store ? store.active === true : true`) em `services/search.service.ts` e `services/product.service.ts`; `getProductComparison(productId)` (identidade do produto no caminho público por UUID — ofertas/preços/summary/best-deal já protegidos); `PriceHistoryQueryService` (mesmo padrão, sem consumer call-site atual); métricas históricas de `market_changes`/volatilidade e o ticker/pulse sem recorte integral por loja ativa; política de cleanup dos dados QA (decisão separada do owner — **nada foi apagado**).
- **Status**: P2 **CLOSED — validated in production for the tested public surfaces**. Isso **não** significa que todas as superfícies possíveis estejam livres de vazamento (ver follow-ups) nem que o E2E tenha sido 25/25 automatizados. Registro técnico detalhado: `docs/operations/MERCHANT_CONSOLE_PRODUCTION_VALIDATION.md`.

## 2026-09-09 — MERCHANT CONSOLE: AUTHORIZATION + CHECKSUM HARDENING (PROD)

**Objetivo**: fechar duas falhas reais encontradas na validação autenticada de produção do Merchant Console, com correção mínima server-side + regressão, e validar o fluxo em produção.

- **`7210ec7` — P1 authorization gap**: `app/api/merchant/imports/commit` autorizava mutação de catálogo apenas com `merchant_stores` membership + role. Agora exige também `merchant_authorizations` **ACTIVE** para o par exato `merchant_id × store_id`, fail-closed (erro de query nunca vira autorização). Mesmo gate aplicado em `imports/run` (sync de conector). Gate puro `canCommitAuthorization` + `MerchantAuthorizationService.hasActiveAuthorization`. Testes: 8 cenários de autorização + 5 fail-closed (`services/__tests__/merchant-console-security.test.ts`).
- **`9f740a8` — checksum contract**: a route comparava o checksum do **raw source** (CSV bruto) com o checksum dos **offers pós-parse** do `CommitService` — representações inerentemente diferentes ⇒ todo import válido retornava `200 FAILED/SOURCE_CHANGED_SINCE_PREVIEW` e **nenhuma oferta era escrita**. Agora há dois níveis explícitos: 1ª camada na route `isRawSourceUnchanged()` (raw imutável ⇒ **409** em conteúdo alterado); 2ª camada `CommitContext.offersChecksum = normalizedOffersChecksum(offers)` (campo renomeado de `sourceChecksum` para eliminar ambiguidade). 4 regressões novas, incluindo a que reproduz o bug.
- **Validação de produção (read-only)**: RLS anônimo bloqueia `merchants`/`merchant_stores`/`merchant_authorizations`/`merchant_import_sessions` (0 linhas; tabela inexistente ⇒ 404 PGRST205, provando que existem). Consumer regression pós-deploy: `/`, `/products`, `/search`, `/search?q=iphone 17 pro`, `/lojas`, `/categorias` = 200. QA invisível no marketplace (`CONSUMER_ACTIVE_STORES=9/11`, QA offers 0, QA products 0).
- **Tenants QA de produção** (owner-executed, idempotente + backup validado): stores A/B inativas, merchants A/B, memberships e autorizações ACTIVE, zero cross-tenant.
- **Regra de domínio confirmada**: `title` **não** é obrigatório (`rowNormalizer` valida `external_id`/preço; vazio ⇒ `"(sem título)"`).
- Gates: **1210** testes (171 suítes), lint 0, typecheck 0, build PASS. Produção intacta (nenhum dado real tocado). Registro completo: `docs/operations/MERCHANT_CONSOLE_PRODUCTION_VALIDATION.md`.
...
**Novo domínio** `src/domains/learning-engine/`: `KnowledgeRecord` (Knowledge Aggregate append-only, nunca UPDATE/DELETE — cada mudança aceita é uma nova versão), `ConfidenceEngine` (puro: promoção local aos ≥10 occurrences confirmadas — `LOCAL_PROMOTION_THRESHOLD` —, promoção global aos ≥2 lojas independentes — `GLOBAL_MIN_INDEPENDENT_STORES` —, mesma árvore de decisão do CONFIDENCE_ENGINE.md), `KnowledgeIngestionService` (lê exclusivamente `merchant_attribute_patterns.resolved_value` confirmado, `catalog_recovery_decisions` já confirmadas, `marketplace_memory_facts` com `validation_status=confirmed` — nunca uma linha pendente/não validada/rejeitada), `GlobalPromotionEngine` (agrega evidência local entre lojas independentes por valor canônico, não por grafia bruta), `KnowledgeObservabilityService` (as 8 métricas da missão: criados, reutilizados, correções automáticas, correções evitadas, tempo economizado, precisão, conflitos, reversões — todas computadas de dados reais, nunca fabricadas).

**Migration** `20260723120000_continuous_knowledge_engine.sql` (aditiva, aguarda SQL Editor): tabela `knowledge_history` — nenhuma tabela existente alterada.

**Compatibilidade**: nenhuma linha de Sync Pipeline, Product Identity, Merge Engine, Offer Ranking, Catalog Recovery, Firewall ou Conectores foi tocada — o Engine só lê essas tabelas, nunca escreve nelas. Reuso ao vivo (o Firewall consultando este ledger antes de decidir) permanece decisão de uma Missão futura, mesmo precedente da própria Marketplace Memory Foundation — provado aqui por simulação/relatório contra dados reais (`scripts/knowledge-engine-validation-report.ts`), nunca editando um arquivo proibido.

**Scripts**: `scripts/knowledge-engine-backfill.ts` (dry-run por padrão) e `scripts/knowledge-engine-validation-report.ts` (somente leitura) — validados em dry-run contra produção: 0 `merchant_attribute_patterns` confirmados, 1003 `catalog_recovery_decisions`, 0 `marketplace_memory_facts` confirmados hoje (números reais, não hipotéticos — nenhuma correção humana foi resolvida via Pending Review ainda em produção).

**Qualidade**: 35 testes novos (123 suítes/812 testes no total, 0 falhas), lint 0, `tsc --noEmit` 0, build OK. Migration criada mas não aplicada (sem mecanismo de DDL neste ambiente — mesmo precedente de toda migration anterior deste projeto); backfill real (`--execute`) e promoção global aguardam a aplicação da migration.

## 2026-07-18 — PROGRAM E-1 — ParaguAI Operating System (Execution Era, Phase I)

Com a Arquitetura, a Estratégia de Marketplace, a Estratégia de Negócio e a Estratégia Corporativa congeladas (Program ΩΩ, Architecture v1.0), o mandato passou de definir estratégia para construir o Sistema Operacional capaz de executá-la de forma repetível por anos. `docs/operations/PARAGUAI_OPERATING_SYSTEM.md` (ADR-059) formaliza: 13 Princípios Operacionais (todos com precedente real nesta história — evidência antes de opinião, medição antes de expansão, confiança antes de receita, entre outros); um Sistema de Planejamento de 10 níveis com autoridade de alteração nomeada por nível; o North Star Score (já vigente em `NORTH_STAR.md` §6 desde 2026-06-27) formalmente comparado com ICE/RICE/WSJF/MoSCoW/Opportunity Scoring e mantido como único framework de priorização, agora com critérios de desempate; um Fluxo de Execução de 8 passos que reconcilia a nomenclatura Program/Mission em uso com Epic/Feature da Foundation; cadência oficial Daily→Yearly; uma Matriz RACI completa de 11 funções (o primeiro mapeamento explícito de responsabilidade do projeto, hoje acumulado por um único operador); um Sistema de KPIs em 4 camadas; um processo formal de Gestão de Riscos (inexistente até hoje); padrões de documentação (nomeando RFC, Runbook e Playbook como tipos ainda não usados); um Sistema de Qualidade estendido a Produto, Marketplace, Comercial e Operações; um Sistema de Aprendizado universal; e Manuais de CEO e Board.

Decisão explícita de não criar nova categoria do Knowledge System nem inserir em `docs/foundation/` (LOCKED) — o POS vive em `docs/operations/` como documento vivo, subordinado à Foundation em toda a extensão, sem reabrir nenhuma decisão de arquitetura, marketplace ou negócio já aceita. Nenhum código, schema ou domínio de aplicação alterado.

## 2026-07-17 — PROGRAM ΩΩ — Engineering Closure (Architecture v1.0)

Encerramento formal da Engenharia v1. Após os Programas Κ (Product Identity/Taxonomy/Merge Engine), Π (Product Identity V2/Knowledge Graph), Ω (Marketplace Memory/Read-Through/Rollout real), e a sequência de auditoria de negócio Λ→Ο (Marketplace Impact Audit, Comparable Coverage Root Cause Analysis, Unknown Match Analysis, Comparable Coverage Expansion Strategy), o Architecture Review Board conduziu duas missões de validação final (ΩΩ-1 Final Architecture Validation, ΩΩ-2 Engineering Sign-Off) e concluiu **ENGINEERING APPROVED WITH MONITORING** — nenhuma falha estrutural, nenhuma reescrita necessária.

Mission ΩΩ-3 (Engineering Closure) formalizou o encerramento: `docs/engineering/ENGINEERING_CONSTITUTION.md` (princípios permanentes, componentes congelados, critérios de reabertura), `docs/architecture/ARCHITECTURE_STATUS.md`/`ENGINEERING_DECISIONS.md`/`ENGINEERING_TIMELINE.md`/`ARCHITECTURE_CERTIFICATE.md`, `CHANGELOG.md`/`VERSION.md`/`ROADMAP.md`/`RELEASE_NOTES.md` na raiz do repositório, e atualização do `README.md` com a seção Engineering Status. Nenhum código alterado — documentação e cerimônia de encerramento apenas.

Achado que orienta a fase seguinte (Era Marketplace v2): o gargalo do ParaguAI deixou de ser algoritmo ou infraestrutura (Product Identity maduro — Ν-1 — e Marketplace Memory validada — Ω-4) e passou a ser composição real de catálogo (Λ-1, confirmado por Μ-1 e Ο-1) — Comparable Product Coverage em 0,08% no encerramento, com teto de curto prazo mapeado em 0,45% sem nenhum produto novo.

## 2026-07-06 — PROGRAM Z — RC-7 — Home Freeze (v1.2) + desenvolvimento por Sprint de componente isolado

Missão de governança, zero código alterado. Após RC-5 (realinhamento contra novo export do v0) e RC-6 (restauração do sistema de largura/containers a 1600px, incluindo `Navbar`, e correção do preenchimento vertical de `StoreCarousel`/`CategoriesCard`), o CTO aprovou o resultado como artefato de produção e determinou que toda evolução visual futura da Home ocorra exclusivamente por Sprint isolada de um único componente por vez.

`docs/design/DESIGN_CONSTITUTION.md` avançou para v1.2 (histórico de versões atualizado, §2 passou a nomear explicitamente `components/layout/Navbar.tsx`/`Footer.tsx` como parte da superfície congelada, novo §7 formaliza a sequência fixa de 8 passos de uma Sprint de componente). Criado `docs/design/HOME_COMPONENTS.md` — registro vivo de todos os 23 componentes visuais da Home (status, última alteração, responsável, dependências, impacto de blast radius), atualizado a cada Sprint futura. Registrado ADR-053 (`docs/operations/DECISIONS.md`) formalizando ambas as decisões.

## 2026-07-06 — Release 1.9 — Program F — Premium Home Experience, Wave 2 (v0 realignment)

Realinhamento visual da Home contra o export oficial do v0.app (`premium-home-interface-mRmShqDN4kh`), aprovado pelo CTO como nova referência — supersede a Wave 1 do congelamento (ADR-050, `docs/design/DESIGN_CONSTITUTION.md` v1.0 → v1.1). Auditoria prévia (componente por componente, `app/page.tsx` + `components/home/**` vs. export) mostrou divergência de sistema completo, não pontual: paleta OKLCH própria (`brand-blue`/`brand-cyan`/`brand-purple`/`positive`/`negative`/`amber`), par tipográfico Sora+Inter, hero fotográfico (Ponte da Amizade) em vez do globo CSS/SVG anterior, e reestruturação do dashboard em duas fileiras (4 cards + 3 colunas) mais uma faixa de confiança/CTA de lojista nova.

Toda a integração de dados real foi preservada e, em dois pontos, aprofundada: `MarketPulseCard` passou a usar os `topDrops`/`topGains` reais já computados por `getMarketPulseHighlights()` (antes calculados e nunca exibidos); `CambioCard` ganhou o par USD→PYG completo (histórico + variação), simétrico ao USD→BRL já existente; `getFeaturedStores()` passou a expor `rating` diretamente, eliminando o segundo `getStoreBySlug()` por loja que `StoreCarousel.tsx` fazia (dívida já registrada em `HOME_AUDIT_2026_07_06.md`). O card de Câmeras ao Vivo manteve deliberadamente o estado honesto "Em breve" (decisão do CTO) — o export do v0 mostra thumbnails estáticos com badge permanente "Ao vivo" sem feed real por trás, o que violaria a regra de nunca apresentar dado fictício como real. Seis seções sem equivalente no export (Offers, EconomiaDoDia, AIShowcase, Benefits, HowItWorks, Brands, ForLojistasSection, CTASection) foram mantidas inalteradas, ancoradas após o bloco reconstruído. Fontes Sora/Inter carregadas apenas em `app/page.tsx` (escopadas ao `<main>` via `--font-home-*`), sem tocar o layout raiz — nenhuma outra página do site mudou de fonte ou paleta.

## 2026-07-06 — PROGRAM Z — Production Home Verification (RC-4) — RESOLVIDO

Missão de auditoria (zero código alterado) aberta porque a produção supostamente continuava exibindo a Home antiga apesar do deploy do commit `d145a9f`. Auditoria via `git log`/`git diff`/`git show` confirmou que o commit `2503c15` (Premium Home Experience) está presente e intacto em `main`/`origin/main`, é o único commit que já tocou `app/page.tsx`/`components/home/**`/`app/categorias/**`/`lib/home-premium-service.ts`, e os 5 commits posteriores (`007c62c`→`d145a9f`) são exclusivamente docs/workflows de CI, sem qualquer alteração de rota, componente ou service — ou seja, **o código-fonte nunca regrediu para a Home antiga**.

A causa aparente estava na camada de deploy, não no repositório. Usando o Vercel CLI (`vercel ls`/`vercel inspect`) confirmou-se que o deployment `dpl_CMySeJmN6TWmiBjr1Mw9cyKy8Rwa` — criado 1m41s depois do commit `d145a9f` — é o deployment `Production` atual, aliado aos domínios reais (`www.fronteiraai.com`, `fronteiraai.com`). Um `curl` direto contra `https://www.fronteiraai.com/` (sem cache de navegador, headers `Cache-Control: no-cache`) confirmou HTTP 200 com os marcadores exclusivos da Home Premium ("Market Pulse", "Live Marketplace", do `DashboardStrip`) presentes no HTML servido, com `X-Vercel-Cache: STALE`/`Age` consistente com uma geração ISR (`revalidate=60`) posterior ao deploy — não um resíduo de antes dele.

**Causa raiz mais provável do que foi observado**: janela de propagação entre o push/commit e o deployment ficar `Ready` (poucos minutos), combinada a cache de navegador (aba aberta antes do novo deploy). **Nenhuma ação de código foi necessária** — produção já serve a Home Premium corretamente. Nenhuma regra de governança nova; ADR-051 (Wave Definition of Done) já cobre o cenário de garantir Deploy verificado antes de declarar uma Wave concluída.
