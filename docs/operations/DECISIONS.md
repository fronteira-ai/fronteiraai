...
## ADR-056 — Program Φ (Phi) — Mission Φ-1: Continuous Comparability Optimization

**Data**: 2026-07-16
**Status**: Aceita

**Contexto**: a missão foi recebida rotulada "Program Σ — Mission Σ-1" — colisão real com o Program Σ já enviado (`docs/architecture/CONNECTOR_PLATFORM_V3.md`, Mission Σ-1, commit `ea1d4bc`; Mission Σ-2 Delta Engine, commit `2062ab9`). Renomeada para **Program Φ (Phi) — Mission Φ-1**, sem alterar nenhum artefato do Program Σ anterior.

**Decisão 1 — nenhuma categoria nova, nenhuma migration**: auditoria e documentação estratégica pura (`docs/product/MARKETPLACE_OPTIMIZATION.md`, `MARKETPLACE_KPIS.md`, `MARKETPLACE_FEEDBACK_LOOP.md`, `docs/architecture/MARKETPLACE_PIPELINE.md`, `MARKETPLACE_DASHBOARD.md`), zero código de produto alterado, zero migration criada ou aplicada. Duas verificações pontuais read-only foram executadas contra produção (contagem de `merge_executions`/`merge_candidates` por status; classificação intra vs. cross-merchant da fila pendente de `merge_candidates`) — mesmo padrão já estabelecido pelos scripts certificados (`cpc-report.ts`, `merge-audit-report.ts`), descartadas após uso, não persistidas como scripts novos.

**Decisão 2 — achado central, medido**: dos 3.072 `merge_candidates` pendentes hoje, **zero são cross-merchant** (3.056 intra-merchant, 16 indeterminado) — executar a fila inteira do jeito que está não move o Comparable Product Coverage (CPC, hoje 0,03%). A causa raiz já tem correção construída e validada por simulação (Program Κ, Mission Κ-3, `ProductSignatureExtractor` — +199 produtos comparáveis projetados sobre ~1,3M pares reais), mas essa camada não está wired ao `ProductIdentityEngine` em produção. Mesma leitura para a Universal Taxonomy (Mission Κ-2): migration pronta, backfill não autorizado.

**Decisão 3 — veredito**: **FOUNDATIONAL OPTIMIZATION GAPS DETECTED** — não por ausência de engenharia, mas porque três peças já construídas e quality-gated (Universal Taxonomy backfill, Product Signature wiring, escala de execução de merge) aguardam autorização explícita do CTO para sair de "construído" para "em produção". Recomendação registrada: autorizar o wiring do Product Signature antes de qualquer nova iniciativa de marketplace — é a única ação, dentre as três, com efeito direto e já medido sobre CPC.

## ADR-057 — Program Κ — Mission Κ-4: Product Identity Integration

**Data**: 2026-07-16
**Status**: Aceita

**Contexto**: Mission Φ-1 identificou que Universal Taxonomy (Κ-2) e Product Signature (Κ-3) — ambas construídas, testadas e validadas por simulação — nunca foram conectadas ao fluxo real de `ProductIdentityEngine`/`CanonicalMergeSuggestionService`, e que essa desconexão é a causa raiz direta de 0 candidatos cross-merchant em 3.072 pendentes. Esta Mission fecha exatamente esse gap — wiring puro, nenhum algoritmo/peso/threshold/score alterado.

**Decisão 1 — único ponto de wiring, dois novos métodos pequenos**: `src/domains/product-identity/services/CanonicalMergeSuggestionService.ts` (já documentado como "a única ponte" entre os domínios desde Program Ω) passa a resolver `categoryId` → `categories.slug` real (novo método `ICanonicalCatalogRepository.findCategorySlugsByIds`, batch, implementado em `SupabaseCanonicalCatalogRepository`) → nó da Universal Taxonomy (`findNodeByRealCategorySlug`, Κ-2, já existia) para o gate de categoria; e substitui `specifications` bruto pela saída achatada de `buildProductSignature` (Κ-3, já existia) para o fator de especificações. `ProductIdentityEngine.ts` permanece byte-a-byte idêntico — confirmado por `git diff` não tocar o arquivo.

**Decisão 2 — fallback de segurança no gate de categoria**: quando o lookup em lote não encontra o slug real de um `categoryId` (linha órfã/lookup incompleto), o código cai para o próprio `categoryId` como valor do gate — nunca para `""`, que colapsaria produtos não resolvidos de categorias diferentes num mesmo valor e criaria falsos positivos. Reproduz exatamente o comportamento antigo (igualdade de UUID) nesse caso-limite.

**Decisão 3 — resultado medido (real) vs. simulado (nunca persistido)**: o wiring em si está em produção assim que este commit for enviado (código real). Sua verificação, porém, rodou como simulação disciplinada — `CanonicalMergeSuggestionService` real e inalterado, importado sem reimplementação, executado para os 17.991 `canonical_products` ativos com repositórios em memória (dados reais, `create()` nunca grava em produção) — mesma disciplina de `scripts/kappa3-cross-merchant-simulation.ts`. Resultado: candidatos cross-merchant **0 → 66** (de 1.920 candidatos ≥70% recomputados do zero; 59 Alta/863 Média/998 Manual), 130 produtos canônicos distintos tocados. Projeção (rotulada como tal, nunca combinada com o número real) de Comparable Coverage se as 66 duplas fossem aprovadas e executadas: 64 grupos com 2+ lojas, 2 com 3+. CPC real de produção permanece 0,03% — reconfirmado após a verificação, porque nada foi escrito.

**Decisão 4 — dois gaps do Program Κ nomeados, não fechados por esta Mission**: Brand Normalization (Κ-2, `normalizeBrandName`/`KNOWN_BRAND_DUPLICATES`) e `ATTRIBUTE_DICTIONARY` (Κ-2) permanecem não conectados — fora do escopo explícito de Objetivo 2/3 (Product Signature + Universal Taxonomy de categoria, não marca). `ProductIdentityShadowStage` (avaliação em tempo de sync, por oferta) permanece um caminho paralelo não wired — documentado como duplicidade real em `docs/engineering/PRODUCT_IDENTITY_PIPELINE.md`, não gera `merge_candidates`, fora do escopo. Migration `20260715140000_universal_taxonomy.sql` continua não aplicada — não era necessária para este wiring (árvore estática em código, zero I/O).

**Decisão 5 — commit/push/tag de encerramento do Program Κ NÃO executados nesta ADR**: a Mission explicitamente condiciona commit final, push e tag de encerramento à autorização explícita do CTO, mesmo com Quality Gate 100% verde (lint 0, typecheck 0, 681/681 testes, build OK) e resultado medido positivo. Nenhuma dessas três ações foi tomada — aguardando autorização.

**Consequência**: `src/domains/product-identity/services/CanonicalMergeSuggestionService.ts` alterado; `ICanonicalCatalogRepository`/`SupabaseCanonicalCatalogRepository` ganham `findCategorySlugsByIds`; 10 arquivos de teste atualizados para o novo método de interface (nenhuma asserção de comportamento alterada, só o mock ampliado); 3 documentos novos (`PRODUCT_IDENTITY_INTEGRATION.md`, `PROGRAM_K_CLOSURE.md`, `PRODUCT_IDENTITY_PIPELINE.md`) mais esta entrada. Nenhuma migration, nenhum `merge_candidates` real escrito, nenhum outro domínio tocado.

## ADR-058 — Program Κ — Mission Κ-5: Program Κ Final Closure

**Data**: 2026-07-16
**Status**: Aceita

**Contexto**: Mission Κ-4 fechou a integração central de Program Κ (Universal Taxonomy + Product Signature wired), mas deixou 2 itens explicitamente em aberto: `ATTRIBUTE_DICTIONARY` (Κ-2, aparentemente sem consumidor) e Brand Normalization (Κ-2, impacto medido em ~0,5% dos rows de `brands`, fora do escopo operacional). Esta Mission existe só para decidir definitivamente o destino desses dois itens e encerrar Program Κ — nenhuma funcionalidade nova, nenhum algoritmo/peso/threshold/score alterado, nenhuma migration.

**Decisão 1 — `ATTRIBUTE_DICTIONARY` mantido como código, com uma autocorreção registrada**: a auditoria inicial desta Mission buscou consumidores só em `src/` e concluiu erroneamente "zero consumidores", chegando a apagar o arquivo e sua exportação pública antes de qualquer commit. Uma segunda verificação obrigatória (buscar também em `scripts/`, antes de qualquer remoção de código real) encontrou o consumidor real: `scripts/kappa2-taxonomy-backfill.ts` importa `ATTRIBUTE_DICTIONARY` do índice público do domínio para popular a tabela `attribute_dictionary` quando a migration `20260715140000_universal_taxonomy.sql` (ainda não aplicada) for autorizada. A remoção foi revertida antes de qualquer commit; a decisão final é manter o arquivo exatamente como está. `docs/engineering/ATTRIBUTE_DICTIONARY.md` foi corrigido de 10 para 13 entradas — divergência real entre doc e código, descoberta durante esta mesma auditoria (Κ-3 adicionou 3 chaves ao TS em 2026-07-15 sem atualizar o doc).

**Decisão 2 — Brand Normalization: função mantida (preparada para expansão futura), lista hardcoded removida (provada redundante)**: remedição real sobre as 852 linhas de `brands` mostra que `normalizeBrandName()` sozinha já colapsa os 2 únicos grupos de duplicata reais do catálogo — a mesma cobertura que `KNOWN_BRAND_DUPLICATES` (lista hardcoded) afirmava fornecer como "exceção que a função sozinha não captura", uma afirmação que a medição prova falsa. Simulação do efeito real de wireá-la (mesmo padrão de Κ-4, isolado ao gate de marca): 28 `canonical_products` envolvidos, 3 merge candidates possíveis, **0 cross-merchant** — não move Comparable Coverage hoje. `normalizeBrandName` permanece exportada, não wired, com essa justificativa medida registrada; `KNOWN_BRAND_DUPLICATES` e os tipos `BrandDuplicateGroup`/`MapConfidence` (este último, zero consumidores de qualquer tipo) foram removidos.

**Decisão 3 — nenhuma alteração fora de `src/domains/taxonomy/`**: `ProductIdentityEngine.ts`, `MergeExecutorService.ts`, `MergeAuditService.ts`, `CanonicalMergeSuggestionService.ts` (o wiring de Κ-4), `OpportunityEngine.ts`, `buyer-intelligence/`, `exchange/` — todos intocados por esta Mission, conforme restrição explícita do brief.

**Decisão 4 — veredito**: **PROGRAM Κ OFFICIALLY COMPLETE**. Checklist completo em `docs/engineering/PROGRAM_K_FINAL_REPORT.md` — os únicos itens não 100% "SIM" são duplicidade (`ProductIdentityShadowStage`, nomeada, aceita, fora do escopo desta Mission por restrição explícita do brief) e Comparable Coverage em produção (mecanismo comprovado, execução em escala é decisão humana futura, não uma falha de integração).

**Decisão 5 — commit/push/tag/roadmap condicionados à autorização explícita do CTO**: mesma disciplina de ADR-057 — nenhuma dessas ações executada nesta ADR.

**Consequência**: `src/domains/taxonomy/data/brand-normalization.ts`, `src/domains/taxonomy/types/taxonomy.types.ts`, `src/domains/taxonomy/index.ts`, `src/domains/taxonomy/__tests__/brand-normalization.test.ts` alterados; `docs/engineering/ATTRIBUTE_DICTIONARY.md` corrigido; `docs/engineering/PROGRAM_K_CLOSURE.md` atualizado; `docs/engineering/PROGRAM_K_FINAL_REPORT.md` criado; esta entrada. Quality Gate: lint 0, typecheck 0, 681/681 testes, build OK. Nenhuma migration, nenhum outro domínio tocado.

## ADR-059 — Program E-1 — Mission E-1: ParaguAI Operating System (POS)

**Data**: 2026-07-18
**Status**: Aceita

**Contexto**: a Engenharia v1 foi encerrada em 2026-07-17 (Program ΩΩ, ainda não commitado nesta sessão — `VERSION.md`, `docs/engineering/ENGINEERING_CONSTITUTION.md`) e a Arquitetura, a Estratégia de Marketplace, a Estratégia de Negócio e a Estratégia Corporativa foram declaradas congeladas. O mandato recebido — atuar como Chief of Staff/COO/CEO/PMO/VPs de Produto, Growth, Comercial, CS, Finanças, Jurídico, IA, Dados e Operações — pediu explicitamente a criação de um Sistema Operacional de execução, não de nova estratégia. Renomeado para **Program E-1 (Execution Era, Phase I)** para não colidir com a nomenclatura grega em uso nos Programs Κ–ΩΩ.

**Decisão 1 — documento único em `docs/operations/`, não uma nova categoria e não `docs/foundation/`**: `docs/operations/PARAGUAI_OPERATING_SYSTEM.md` foi escolhido em vez de (a) criar uma nova categoria do Knowledge System — rejeitado por exigir uma ADR só para autorizar a categoria, desproporcional ao conteúdo; (b) inserir em `docs/foundation/` — rejeitado porque essa categoria é LOCKED, tem 8 documentos permanentes com expansão sujeita a aprovação de CTO própria (`CLAUDE.md`), e o conteúdo do POS é operacional (cadência, RACI, KPIs, playbooks), não principiológico; (c) dividir em ~10 documentos separados (um por Parte do brief) — rejeitado por violar o anti-pattern já nomeado em `docs/foundation/FOUNDATION_INDEX.md` §XII ("criar documento novo sem necessidade clara"; "cada documento adicionado aumenta o custo de manutenção"), quando as 15 Partes pedidas formam um sistema coeso e nenhuma seção seria consultada de forma verdadeiramente independente das outras.

**Decisão 2 — o POS é explicitamente subordinado à Foundation e não reabre nenhuma estratégia**: o documento declara em seu §0 que não é LOCKED, não cria uma nova hierarquia de decisão paralela à de `NORTH_STAR.md` §5, e cede em qualquer conflito real com a Foundation ou com ADR já aceito. Nenhuma seção altera `BUSINESS_MODEL.md`, `VISION_2035.md`, o roadmap de negócio (`ROADMAP.md`) ou qualquer ADR de arquitetura/marketplace/comercial já aceito.

**Decisão 3 — o framework de priorização não foi substituído**: a Parte 3 do brief pedia comparação explícita entre ICE, RICE, WSJF, MoSCoW e Opportunity Scoring com escolha justificada. A decisão foi manter o North Star Score já definido em `NORTH_STAR.md` §6 (vigente desde 2026-06-27) como único framework de topo — por ser estruturalmente um RICE adaptado à métrica North Star real do projeto (troca "Reach", não aplicável na fase atual de composição de catálogo, por "Geração de Ativo" e "Reutilização", os dois critérios mais validados nesta história via Program Λ→Ο) — e não introduzir um segundo framework concorrente. MoSCoW foi formalizado como ferramenta complementar de negociação de escopo *dentro* de uma Mission já aprovada, não como framework de priorização entre iniciativas. Critérios de desempate (ausentes em `NORTH_STAR.md` até esta ADR) foram adicionados no POS §4.

**Decisão 4 — RACI é um modelo-alvo, não uma ficção de equipe que não existe**: a matriz de 11 funções × 11 papéis (POS §7) documenta explicitamente que hoje um único operador acumula a maioria dos papéis, com Missions executadas por agentes de IA sob mandato — a matriz define a distribuição que passa a valer papel por papel à medida que cada posição é efetivamente contratada, sem exigir redesenho quando isso acontecer.

**Decisão 5 — nenhum processo novo inventado sem lastro em evidência real do projeto**: cada um dos 13 Princípios Operacionais (POS §2) cita um Program, ADR ou artefato real já existente como exemplo — nenhum princípio é aspiracional sem precedente. O Sistema de Gestão de Riscos (POS §9) e os tipos de documento Runbook/RFC/Playbook (POS §10) são nomeados como lacunas reais e explicitamente não preenchidos retroativamente (aplicação do próprio Princípio 4 — simplicidade antes de complexidade: criar sob demanda, não por antecipação).

**Decisão 6 — nenhum código, schema ou domínio de aplicação tocado**: Mission inteiramente de documentação e governança, seguindo o mesmo padrão de missões anteriores de encerramento/constituição (Program ΩΩ, `INCREMENTAL_ARCHITECTURE_CONSTITUTION.md`). Nenhuma migration, nenhum teste, nenhum arquivo em `src/` alterado.

**Consequência**: `docs/operations/PARAGUAI_OPERATING_SYSTEM.md` criado (15 Partes); esta entrada em `docs/operations/DECISIONS.md`; entrada correspondente em `docs/operations/CHANGELOG.md`; `docs/foundation/FOUNDATION_INDEX.md` atualizado para registrar o novo documento vivo. Nenhum outro arquivo tocado por esta Mission.
