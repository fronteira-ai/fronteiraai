...
## Fundação de dados (Sprint 3.7) — limitações conhecidas

- **Seed implementado, não testado contra escrita real** — `database/seed/index.js` foi validado em dry-run (lê corretamente o estado atual e planeja as ações certas), mas nunca rodou com `--execute` contra o Supabase. Risco concreto: a chave usada hoje (`NEXT_PUBLIC_SUPABASE_ANON_KEY`) pode não ter permissão de `INSERT`/`UPDATE` se as policies de RLS das tabelas só permitirem leitura pública — nesse caso, o seed precisaria de `SUPABASE_SERVICE_ROLE_KEY` (variável nova, nunca exposta como `NEXT_PUBLIC_*`) configurada em `.env.local`. Não verificado nesta sprint porque verificar exigiria tentar escrever, e nenhum insert foi autorizado.
- **Detecção de oferta "órfã" em `validate.js` é por `IS NULL`, não por anti-join real** — com 0 linhas hoje, suficiente; não escala para detectar uma FK que aponta para um `id` que não existe mais (deletado), só para uma FK nunca preenchida. Quando houver volume real, isso precisa de uma consulta dedicada (ou um RPC) — fetch-and-diff em JavaScript não escala para milhões de linhas.
- **`price_history`, Price Engine e Offer Ranking são arquitetura, não código** — ADR-013/ADR-014 (`docs/DECISIONS.md`) descrevem a direção, mas nenhuma tabela, função ou ordenação nova foi implementada. `getOffersByProduct`/`getOffersByStore` continuam ordenando só por `price_usd`.
