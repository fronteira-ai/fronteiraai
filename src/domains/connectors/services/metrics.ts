import type { PipelineContext, StageMetrics, PipelineError, PipelineMetrics } from "../types/pipeline.types";

export function initMetrics(connectorId: string, batchId: string): PipelineMetrics {
  return {
    connectorId,
    batchId,
...
export function printReport(ctx: PipelineContext): void {
  ...
  console.log("\n" + "═".repeat(60));
  console.log(`  CONNECTOR SYNC REPORT — ${m.connectorId}`);
