...
export function getServiceClient(): SupabaseClient {
  ...
  if (!url || (!serviceKey && !anonKey)) {
    throw new Error(
      "NEXT_PUBLIC_SUPABASE_URL e SUPABASE_SERVICE_ROLE_KEY são necessários para o Acquisition Engine."
    );
  }

  if (!serviceKey) {
    console.warn(
      "[acquisition] SUPABASE_SERVICE_ROLE_KEY ausente — usando a chave anônima. " +
