...
import { BuyerEventBrainBridgeService } from "@/src/domains/merchant-analytics/services/BuyerEventBrainBridgeService";
...
export function createAnalyticsServices(client: SupabaseClient) {
  ...
  const brainBridge = new BuyerEventBrainBridgeService(client);

  return {
    eventPlatform: new EventPlatformService(eventRepo, sessionRepo, brainBridge),
